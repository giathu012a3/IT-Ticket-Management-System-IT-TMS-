...
def leader_dashboard():
    ...
    try:
        ...
        if cur_start:
            query = query.filter(
                Ticket.created_at >= cur_start, Ticket.created_at < cur_end
            )

        waiting_assignment_count = (
            Ticket.query.join(TicketStatus)
            .filter(TicketStatus.name == "New", Ticket.assigned_to_id == None)
            .count()
        )

        assigned_count = (
            Ticket.query.join(TicketStatus)
            .filter(TicketStatus.name.in_(["Assigned", "In Progress", "Waiting"]))
            .count()
        )
        ...
        sla_warnings = (
            ...
            .filter(
                or_(
                    ...
                    & (Ticket.created_at < warning_time_new),
                    (TicketStatus.name == "In Progress")
                    & (Ticket.updated_at < warning_time_progress),
                )
            )
            .all()
        )
        ...
        if cur_start:
            resolved_query = resolved_query.filter(
                Ticket.updated_at >= cur_start, Ticket.updated_at < cur_end
            )
        ...
        delta_resolved = 0
        if prev_start:
            prev_total = Ticket.query.filter(
        ...
        for staff in staff_members:
            # Active (Real-time snapshot) - includes Assigned, In Progress, Waiting
            active_count = (
                Ticket.query.join(TicketStatus)
                .filter(
            ...
            staff_stats.append(
                {"name": name, "active": active_count, "resolved_month": resolved_count}
            )
...
def get_date_ranges(time_range):
    ...
    if time_range == "7d":
        ...
        if cur_start.month == 1:
            prev_start = cur_start.replace(
                year=cur_start.year - 1,
                month=12,
                day=1,
                hour=0,
                minute=0,
...
def all_tickets():
    ...
    if search:
        query = query.filter(
            or_(Ticket.title.ilike(f"%{search}%"), Ticket.id.ilike(f"%{search}%"))
        )

    page = request.args.get("page", 1, type=int)
    tickets_pagination = query.order_by(Ticket.created_at.desc()).paginate(
        page=page, per_page=10, error_out=False
    )
    statuses = TicketStatus.query.all()

    return render_template(
        "leader/all_tickets.html",
...
def assign_ticket(ticket_id):
    ...
    if staff_id:
        ...
        n = Notification(
            user_id=staff_id,
            message=f"Bạn được phân công yêu cầu: {ticket.title}",
        ...
        n_user = Notification(
            ...
            message=f"Yêu cầu đã được phân công cho kỹ thuật viên",
            link=url_for("user.view_ticket", ticket_id=ticket.id),
        )
